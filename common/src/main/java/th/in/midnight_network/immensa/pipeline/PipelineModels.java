...
public final class PipelineModels implements AutoCloseable {
    ...
    public static synchronized void load() {
        ...
        Thread t = new Thread(() -> {
            try {
                LOG.info("Loading immensa ML models (background)...");
                ...
                loadFailure = e;
                loadStarted = false;
                LOG.error("Failed to load immensa models", e);
            } finally {
                loadDone.countDown();
            }
        }, "immensa-models");
        ...
        if (latch != null) {
            try {
                if (!latch.await(10, TimeUnit.MINUTES)) {
                    throw new RuntimeException("Timed out waiting for immensa models");
                }
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new RuntimeException("Interrupted waiting for immensa models", e);
            }
        }
        if (INSTANCE == null) {
            Throwable failureCause = loadFailure;
            if (failureCause != null) {
                throw new IllegalStateException("Failed to load immensa models", failureCause);
            }
    ...
    public synchronized void close() {
        ...
        LOG.info("Closing immensa ML models");
        try {
            coarseModel.close();
            baseModel.close();
            decoderModel.close();
