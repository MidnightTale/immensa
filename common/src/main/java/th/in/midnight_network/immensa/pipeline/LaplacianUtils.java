...
public final class LaplacianUtils {
    ...
    /**
     * Bilinearly resizes a flat channel and returns only a crop of the resized image.
     *
     * <p>This is mathematically identical to {@link #bilinearResize(float[][], int, int)}
     * with {@code align_corners=false}, but it avoids materializing the complete
     * upscaled image. Terrain tiles normally need a small interior crop from a
     * padded source, so this removes several multi-megabyte temporary arrays per
     * elevation/climate tile.</p>
     */
    public static float[] bilinearResizeCrop(float[] src, int srcOffset,
                                             int srcH, int srcW,
                                             int dstH, int dstW,
                                             int cropRow, int cropColumn,
                                             int cropH, int cropW) {
        if (srcH <= 0 || srcW <= 0 || dstH <= 0 || dstW <= 0
                || cropH < 0 || cropW < 0) {
            throw new IllegalArgumentException("Invalid resize or crop dimensions");
        }
        int required = srcOffset + srcH * srcW;
        if (srcOffset < 0 || required > src.length) {
            throw new IllegalArgumentException("Source channel exceeds flat array");
        }

        float[] result = new float[cropH * cropW];
        float rowScale = (float) srcH / dstH;
        float columnScale = (float) srcW / dstW;
        for (int r = 0; r < cropH; r++) {
            int dstR = Math.max(0, Math.min(dstH - 1, cropRow + r));
            float sourceR = (dstR + 0.5f) * rowScale - 0.5f;
            int rawR0 = (int) Math.floor(sourceR);
            int r0 = Math.max(0, Math.min(srcH - 1, rawR0));
            int r1 = Math.max(0, Math.min(srcH - 1, rawR0 + 1));
            float rowWeight = sourceR - rawR0;
            int row0 = srcOffset + r0 * srcW;
            int row1 = srcOffset + r1 * srcW;

            for (int c = 0; c < cropW; c++) {
                int dstC = Math.max(0, Math.min(dstW - 1, cropColumn + c));
                float sourceC = (dstC + 0.5f) * columnScale - 0.5f;
                int rawC0 = (int) Math.floor(sourceC);
                int c0 = Math.max(0, Math.min(srcW - 1, rawC0));
                int c1 = Math.max(0, Math.min(srcW - 1, rawC0 + 1));
                float columnWeight = sourceC - rawC0;

                float top = src[row0 + c0]
                        + (src[row0 + c1] - src[row0 + c0]) * columnWeight;
                float bottom = src[row1 + c0]
                        + (src[row1 + c1] - src[row1 + c0]) * columnWeight;
                result[r * cropW + c] = top + (bottom - top) * rowWeight;
            }
        }
        return result;
    }
