...
public final class BiomeClassifier {
    ...
    static final short BEACH = 149, SNOWY_BEACH = 150, STONY_SHORE = 151;
    static final short MANGROVE_SWAMP = 152, FLOWER_FOREST = 153, OLD_GROWTH_PINE_TAIGA = 154;
    ...
    /** Applies coast types and ecotones after the regional climate/geology pass. */
    static void applyRegionalTransitions(short[] biomes, float[] elevation, float[] climate,
                                         short[] landforms, byte[] geology, int h, int w) {
        int n = h * w;
        short[] original = biomes.clone();
        for (int r = 0; r < h; r++) for (int c = 0; c < w; c++) {
            int i = r * w + c;
            float temperature = climate == null ? 14f : climate[i];
            float precipitation = climate == null ? 650f : climate[2 * n + i];

            if ((landforms[i] & TerrainMetadata.COAST) != 0 && elevation[i] > 0) {
                boolean rocky = geology[i] == TerrainMetadata.GEO_GRANITIC
                        || geology[i] == TerrainMetadata.GEO_VOLCANIC
                        || neighborRelief(elevation, r, c, h, w) > 45f;
                if (rocky) biomes[i] = STONY_SHORE;
                else if (temperature < 1f) biomes[i] = SNOWY_BEACH;
                else if (temperature > 19f && precipitation > 1050f) biomes[i] = MANGROVE_SWAMP;
                else biomes[i] = BEACH;
                continue;
            }

            if (geology[i] == TerrainMetadata.GEO_VOLCANIC && elevation[i] > 650) {
                biomes[i] = temperature < 1f ? STONY_PEAKS : BADLANDS;
                continue;
            }

            int unlike = countUnlikeNeighbors(original, r, c, h, w);
            if (unlike == 0) continue;
            biomes[i] = switch (original[i]) {
                case FOREST -> unlike >= 2 ? FOREST_SPARSE : FOREST;
                case TAIGA -> unlike >= 2 ? TAIGA_SPARSE : TAIGA;
                case SNOWY_TAIGA -> unlike >= 2 ? SNOWY_TAIGA_SPARSE : SNOWY_TAIGA;
                case JUNGLE -> precipitation < 1250f || unlike >= 3 ? FOREST_SPARSE : JUNGLE;
                case PLAINS -> precipitation > 850f && unlike >= 2 ? FLOWER_FOREST : PLAINS;
                case DESERT -> precipitation > 260f && unlike >= 2 ? SAVANNA : DESERT;
                case TAIGA_SPARSE -> precipitation > 900f && unlike <= 1 ? OLD_GROWTH_PINE_TAIGA : TAIGA_SPARSE;
                default -> original[i];
            };
        }
    }

    private static int countUnlikeNeighbors(short[] biomes, int r, int c, int h, int w) {
        short center = biomes[r * w + c];
        int unlike = 0;
        if (r > 0 && biomes[(r - 1) * w + c] != center) unlike++;
        if (r + 1 < h && biomes[(r + 1) * w + c] != center) unlike++;
        if (c > 0 && biomes[r * w + c - 1] != center) unlike++;
        if (c + 1 < w && biomes[r * w + c + 1] != center) unlike++;
        return unlike;
    }

    private static float neighborRelief(float[] elevation, int r, int c, int h, int w) {
        float min = Float.POSITIVE_INFINITY, max = Float.NEGATIVE_INFINITY;
        for (int dr = -1; dr <= 1; dr++) for (int dc = -1; dc <= 1; dc++) {
            int nr = Math.max(0, Math.min(h - 1, r + dr));
            int nc = Math.max(0, Math.min(w - 1, c + dc));
            float value = elevation[nr * w + nc];
