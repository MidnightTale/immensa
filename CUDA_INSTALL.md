...
This guide applies to the `-cuda` build of Immensa, and to the `-universal` build on Linux (which loads CUDA when the required libraries are installed). If you are on Windows (DirectML) or use the `-cpu` build, no CUDA setup is required.
